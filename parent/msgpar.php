...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_custom_message'])) {
    if (isset($_SESSION['selected_username']) && !empty($_SESSION['selected_username'])) {
        $sender_username = isset($_SESSION['parent_username']) ? $_SESSION['parent_username'] : '';
        ...
        if (empty($sender_username)) {
            ...
            $sql_insert_custom_message = "INSERT INTO messages2 (sender_id, receiver_id, message, timestamp) VALUES ('$sender_username', '$receiver_username', '$message_content', NOW())";
...
$sql_users = " (SELECT name FROM teacher where action=1)";
...
<body>
     <div class="header">
        <h1>Welcome, <?php echo isset($_SESSION['parent_username']) ? $_SESSION['parent_username'] : 'Guest'; ?>!</h1>
                    ...
                    <div class="messages">
                        ...
                        if (isset($_SESSION['selected_username']) && !empty($_SESSION['selected_username'])) {
                           ...
                           /* $sql_messages = "(SELECT * FROM messages2 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)UNION(SELECT * FROM messages1 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)UNION(SELECT * FROM messages3 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)";*/
                            $sql_messages = "(SELECT * FROM messages2 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)
                UNION
                (SELECT * FROM messages1 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)
                UNION
                (SELECT * FROM messages3 WHERE (sender_id = '$sender_username' AND receiver_id = '$receiver_username') OR (sender_id = '$receiver_username' AND receiver_id = '$sender_username') ORDER BY timestamp)
                ORDER BY timestamp DESC";
                            ...
                            if ($result_messages && mysqli_num_rows($result_messages) > 0) {
                                while ($row = mysqli_fetch_assoc($result_messages)) {
                                    ...
                                    $is_sender = $message_sender === $_SESSION['parent_username'];
...
function closeSessionsAndRedirect() {
    ...
    window.location.href = "parnt1.php"; // Replace "your_file.html" with the path to your file
}
</script>
