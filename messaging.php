...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_custom_message'])) {
    if (isset($_SESSION['selected_username']) && !empty($_SESSION['selected_username'])) {
        ...
        if (empty($sender_username)) {
            ...
            $sql_insert_custom_message = "INSERT INTO messages (sender_id, receiver_id, message, timestamp) VALUES ('$sender_username', '$receiver_username', '$message_content', NOW())";
...
$sql_users = "(SELECT username FROM users)UNION (SELECT name FROM teacher)";
...
function closeSessionsAndRedirect() {
    ...
    window.location.href = "home.php"; // Replace "your_file.html" with the path to your file
}
</script>
